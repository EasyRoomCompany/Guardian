...
export const EasyRoomPage = () => {
  const handleChange = () => {
    return fetch("http://localhost:3333/room")
      .then((res) => res.json())
      .then((res) => {
        console.log(res);
      });
  ...
  const style =
    "group relative w-30 flex justify-center py-2 px-4 border border-transparent text-sm font-medium rounded-md text-white bg-orange-500 hover:bg-orange-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-orange-500";
    ...
    <>
      <div className="flex flex-row items-center justify-center gap-5">
        ...
        <div className="w-80 flex flex-col items-center justify-center h-screen">
          <button className="w-80 mt-5 p-2 rounded-lg text-sm focus:outline-none border border-orange-500">
            Agendar
          </button>
        </div>
      </div>
